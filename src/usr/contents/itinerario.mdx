    ...
    <VectorLayerLibre
      ...
      source={{
        directus: {
          ...
          geoField: "coordinate",
          queryString: `filter[id][_in]=${id_luoghi}&fields=id,tipo_monumento,nome_luogo,coordinate&limit=-1`,
          transType: "geojson",
        }
      }}
      checked={true}
      ...
      popupTemplate="<strong>${nome_luogo}</strong>
        <p>${tipo_monumento}</p>
  ...
  ---
    ...
    <Field name={["luoghi_dell_itinerario"]} transformer={ ldi => {
      ...
      source={{
        directus: {
          table: "luogo_di_interesse",
          queryString: `filter[id][_in]=${id_luoghi}&fields=id,nome_luogo,tipo_monumento,anno_inizio,periodo_fine,caratteristica_di_dettaglio,link_immagine,fa_parte_di.nome_luogo,fa_parte_di.toponimo,fa_parte_di.localita&`
        }
      }}
      striped
      highlightOnHover
